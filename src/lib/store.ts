...
const BUILTIN_SECTIONS: SectionDef[] = [
  ...
  { id: "todos", label: "Todos", builtin: "todos", order: 1 },
  { id: "map", label: "Map", builtin: "map", order: 2 },
  { id: "books", label: "Books", filter: { type: "book" }, order: 3 },
  { id: "codes", label: "Codes", filter: { type: "code" }, order: 4 },
  { id: "images", label: "Images", builtin: "images", order: 5 },
];

async function ensureSeed(existing: SectionDef[]) {
  if (existing.length === 0) {
    for (const s of BUILTIN_SECTIONS) await putSection(s);
  }
}
