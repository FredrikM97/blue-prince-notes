...
type CaptureStoreSlice = ReturnType<typeof useCaptureStoreSlice>;
type CaptureFormState = ReturnType<typeof useCaptureFormState>;

function useCaptureStoreSlice() {
...
function QuickNoteHeader({ mode }: { mode: "note" | "todo" }) {
  return (
...
function QuickNoteDetailsSection({
  mode,
  body,
  setBody,
  showDetailsPreview,
...
function QuickNoteAttachmentsSection({
  setPendingImages,
  showHelp,
  setShowHelp,
}: {
...
function QuickNoteFooterActions({
  submit,
}: {
...
function useCaptureDerivedData({
  gridCells,
  notes,
  todos,
  title,
  ...
  gridCells: CaptureStoreSlice["gridCells"];
  notes: CaptureStoreSlice["notes"];
  todos: CaptureStoreSlice["todos"];
  title: string;
  cursorPos: number;
  room: string;
}) {
...
function useCaptureGlobalEffects({
  open,
  openCapture,
  setPendingImages,
}: {
...
function useCaptureSubmit({
  ...
  create: CaptureStoreSlice["create"];
  close: () => void;
  mode: CaptureFormState["mode"];
  title: CaptureFormState["title"];
  pendingImages: CaptureFormState["pendingImages"];
  body: CaptureFormState["body"];
  type: CaptureFormState["type"];
  room: CaptureFormState["room"];
  tagsInput: CaptureFormState["tagsInput"];
  priority: CaptureFormState["priority"];
  resetAfterSubmit: CaptureFormState["resetAfterSubmit"];
}) {
...
export function QuickNotePanel() {
  const store = useCaptureStoreSlice();
  ...
  const derived = useCaptureDerivedData({
  ...
  return (
    <SidebarPanel open={store.open} onClose={store.close} className="capture-panel">
      <QuickNoteHeader mode={form.mode} />
      <QuickNoteModeTabs mode={form.mode} setMode={form.setMode} />
      ...
      <div className="capture-form-stack">
        ...
        <QuickNoteTagsField tagsInput={form.tagsInput} setTagsInput={form.setTagsInput} />

        <QuickNoteDetailsSection
          mode={form.mode}
          body={form.body}
      ...
      <QuickNoteFooterActions submit={submit} />
    </SidebarPanel>
  );
}
